...
| **exe.dev API key** | Stored in the Shelley settings table (`deploy_api_key`), returned to the UI only masked; validated live via `POST https://exe.dev/exec` (`whoami`). Create with `ssh exe.dev ssh-key generate-api-key --cmds=whoami,ls,new,share,rm --exp=90d` — the `share` and `rm` permissions are needed for proxy routing and VM cleanup. |
