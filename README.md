...
| **exe.dev API key** *(required, saved or entered)* | Stored in the Shelley settings table (`deploy_api_key`), returned to the UI only masked; validated live via `POST https://exe.dev/exec` (`whoami`). Create with `ssh exe.dev "ssh-key generate-api-key --cmds=whoami,ls,new,ssh-key\ add,share\ port,share\ set-public,rm --exp=90d"` — `ssh-key add` registers Shelley’s public deploy key scoped to the `shelley-deploy` tag (applied to every deployer-created VM), `share port` and `share set-public` configure proxy access, and `rm` supports cleanup. The private deploy key stays only on the source Shelley VM. |
